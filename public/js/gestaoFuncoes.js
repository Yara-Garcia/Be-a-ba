...
fetch('http://localhost:3000/functions', {
    ...
    .then(data => {
        if (data.success) {
            ...
            functions.sort((a, b) => a.nome_funcao.localeCompare(b.nome_funcao));
            ...
            functions.forEach(functionData => {
                ...
                const editarIcon = criarSvgIcone('editar', `editarFuncao.html?id_funcao=${functionData.id_funcao}`);
                const deletarIcon = criarSvgIcone('deletar', 'gestaoFuncoes.html');
                deletarIcon.setAttribute('function-id', functionData.id_funcao); // Armazena o id como um atributo do ícone de exclusão
                ...
                deletarIcon.addEventListener('click', function (event) {
                    ...
                    const functionId = this.getAttribute('function-id');
                    confirmarExclusaoFuncao(functionId);
                });
            ...
            const tabelaFuncoes = document.querySelector('.main-content table');
            ...
            function filtrarTabela(filtro) {
                const linhas = tabelaFuncoes.querySelectorAll('tbody tr');
                linhas.forEach(linha => {
                    const nomeFuncao = removerAcentos(linha.querySelector('.col-name').textContent.toLowerCase()); // Normaliza e converte para minúsculas
                    ...
                    if (nomeFuncao.includes(filtro) || descricao.includes(filtro)) {
